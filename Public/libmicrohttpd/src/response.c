 ...
 */
static ssize_t
DownloadFileReaderCallback (void *cls, uint64_t pos, char *buf, size_t max)
{
 ...
 */
static void
DownloadFileFreeCallback (void *cls)
{
  struct MHD_Response *response = cls;
                                         ...
                                         uint64_t offset)
{
  ...
  response = MHD_create_response_from_callback (size,
						4 * 1024,
						&DownloadFileReaderCallback,
						NULL,
						&DownloadFileFreeCallback);
  if (NULL == response)
    return NULL;
  response->fd = fd;
