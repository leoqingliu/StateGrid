...
HRESULT CWMIUtils::Init()
{
	...
	lRet = CoInitializeEx(0, COINIT_MULTITHREADED); 
	if (FAILED(lRet))
	{
...
BOOL CWMIUtils::GetDiskId(CString& strId)
{
	LRESULT lInit = Init();
...
BOOL CWMIUtils::GetBiosId(CString& strId)
{
	LRESULT lInit = Init();
	if (FAILED(lInit) || NULL == m_pSvc)
	{
