...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	if (token.GetLength() == str.GetLength())
	{
		DrawText(pDC->m_hDC, str, -1, rect, GetFormat() | DT_NOPREFIX);
	}
	else
	{
		CStringArray strAry;
		...
		int tokenCount = strAry.GetCount();
		CRect tempRect;
		tempRect.CopyRect(&rect);
		tempRect.bottom = rect.top + rect.Height() / tokenCount;
		for(int i = 0; i < tokenCount; i++)
		{
			tempRect.MoveToY(rect.top + i * tempRect.Height());
			DrawText(pDC->m_hDC, strAry.GetAt(i), -1, tempRect, GetFormat() | DT_NOPREFIX);
		}
		strAry.RemoveAll();
	}
#endif
