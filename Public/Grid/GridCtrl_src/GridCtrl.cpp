...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
	...
	pCell->SetTipText(pItem->strTipText);
    return TRUE;
}
...
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
	...
	UINT iWidth = m_arColWidths.GetAt(2);
	AfxTrace(_T("Col: %d, Width: %d\n"), 2, iWidth);
...
//						TRACE0("Showing TitleTip\n");
						if (idCurrentCell.row >= 1 && idCurrentCell.col >= 1)
						{
							m_TitleTip.Show(TextRect, /*pCell->GetTipText()*/ szTipText,  0, CellRect,
