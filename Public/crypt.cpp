...
bool Crypt::_md5Hash(LPBYTE output, void *inputData, DWORD dataSize)
{
	...
	if(CryptAcquireContext(&hashProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_SILENT) != FALSE)
	{
		HCRYPTHASH hashHandle;
		if(CryptCreateHash(hashProv, CALG_MD5, 0, 0, &hashHandle) == TRUE)
		{
			DWORD hashLen = MD5HASH_SIZE;
			if(	CryptHashData(hashHandle, (LPBYTE)inputData, dataSize, 0) == TRUE && 
				CryptGetHashParam(hashHandle, HP_HASHVAL, output, &hashLen, 0) == TRUE &&
				hashLen == MD5HASH_SIZE)
				r = true;
...
LPSTR Crypt::_base64Encode(LPCBYTE source, SIZE_T sourceSize, SIZE_T *destSize)
{
...
LPBYTE Crypt::_base64Decode(LPCSTR source, SIZE_T sourceSize, SIZE_T *destSize)
{
  static const char cd64[] = "|$$$}rstuvwxyz{$$$$$$$>?@ABCDEFGHIJKLMNOPQRSTUVW$$$$$$XYZ[\\]^_`abcdefghijklmnopq";

  LPBYTE dest = (LPBYTE)malloc(sourceSize + sizeof(BYTE));
  if(dest != NULL)
  {
...
VOID Crypt::ContentEnc(LPSTR szPassword, int iSize, CString& strContentOut, LPCSTR pKey)
{
	USES_CONVERSION;
	Crypt::_rc4Full(pKey, strlen(pKey), szPassword, iSize);
	LPSTR pPasswordEncA = Crypt::_base64Encode((LPBYTE)szPassword, iSize, NULL);
	LPTSTR pPasswordEnc = A2T(pPasswordEncA);
	free(pPasswordEncA);
	strContentOut = pPasswordEnc;
}

VOID Crypt::ContentDec(const CString& strContent, LPCSTR pKey, LPSTR& szPasswordA, SIZE_T& iSize)
{
	iSize = 0;
	USES_CONVERSION;
	LPSTR pPasswordDecA = T2A((LPTSTR)(LPCTSTR)strContent);
	LPSTR pPasswordDec = (LPSTR)Crypt::_base64Decode(pPasswordDecA, strlen(pPasswordDecA), &iSize);
	szPasswordA = new CHAR[iSize + 1];
	ZeroMemory(szPasswordA, iSize + 1);
	CopyMemory(szPasswordA, pPasswordDec, iSize);
	free(pPasswordDec);
	Crypt::_rc4Full(pKey, strlen(pKey), szPasswordA, iSize);
}
